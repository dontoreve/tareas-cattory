...
export default function LoginPage() {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-slate-50 overflow-hidden" style={{ paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      ...
      <div className="shrink-0 bg-white rounded-t-3xl shadow-[0_-4px_20px_rgba(0,0,0,0.06)] px-6 pt-6 pb-6">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-3.5 font-medium text-slate-700 shadow-sm active:bg-slate-50 transition-colors"
        >
          {googleLoading ? (
            "Conectando..."
          ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full rounded-xl bg-primary py-3.5 text-[15px] font-bold text-white active:bg-primary/90 transition-colors disabled:opacity-50"
          >
            {submitting
